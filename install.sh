...
displayErr() {
    ...
    # for mcrypt on php7.2
    sudo apt-get -y install php7.2-dev libmcrypt-dev
    sudo pecl install mcrypt-1.0.1
    echo "extension=mcrypt.so" > sudo tee --append /etc/php/7.2/mods-available/mcrypt.ini
...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    # Compil Blocknotify
    cd ~
    git clone https://github.com/wakiyamap/yiimp.git
